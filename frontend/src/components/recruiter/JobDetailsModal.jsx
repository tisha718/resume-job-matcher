...
const JobDetailsModal = ({ isOpen, onClose, job }) => {
  ...
  // Mock analytics data
  const analytics = {
    total_applications: job.applicants || 0,
    applied: job.applicants || 0,
    screened: Math.floor((job.applicants || 0) * 0.7),
    interviewed: Math.floor((job.applicants || 0) * 0.4),
    offered: Math.floor((job.applicants || 0) * 0.2),
    rejected: Math.floor((job.applicants || 0) * 0.1)
  };
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="bg-green-50 rounded-lg p-3 text-center">
                <p className="text-2xl font-bold text-green-600">{job.strongMatches || 0}</p>
                <p className="text-xs text-gray-600 mt-1">Strong Matches</p>
              </div>
              <div className="bg-yellow-50 rounded-lg p-3 text-center">
                <p className="text-2xl font-bold text-yellow-600">{job.goodMatches || 0}</p>
                ...
                <div className="flex items-center space-x-2 flex-1 max-w-[60%]">
                  <div className="flex-1 h-2.5 bg-gray-200 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ 
                        width: `${analytics.total_applications ? (analytics.screened / analytics.total_applications * 100) : 0}%` 
                      }}
                    />
                  </div>
                  <span className="text-gray-900 font-semibold w-10 text-right">
